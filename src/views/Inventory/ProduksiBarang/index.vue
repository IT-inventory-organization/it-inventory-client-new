<template>
  <div>
    ...
    <v-row no-gutters align="center">
      <v-col cols="11">
        <div class="display-1 font-weight-bold">Produksi Barang</div>
      </v-col>
      <v-col cols="1">
        <button
        ...
        <template v-slot:[`item.action`]="{ item }">
          <v-menu offset-y>
            ...
            <v-list class="it-inventory-actions-list">
              ...
              <v-list-item @click.prevent="handleOpenView(item.dataBarangId)">
                <v-list-item-title>
                  <Icon
    ...
    <v-dialog v-model="dialogView" persistent width="100%" max-width="1200px">
      <produksi-barang-view @handleCloseView="handleCloseView" />
    </v-dialog>
  </div>
</template>
...
export default {
  ...
  components: {
    Icon,
    FormPo: () =>
      import("@/components/inventory/ProduksiBarang/FormProduksiBarang"),
    PurchaseOrderView: () => import("@/components/po/PurchaseOrderView"),
    ProduksiBarangView: () =>
      import("@/components/inventory/ProduksiBarang/ProduksiBarangView"),
  },
  data() {
    return {
      dialogView: false,
